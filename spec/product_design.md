...
- **简单部署**：默认单二进制文件，嵌入式存储和搜索，无外部依赖；生产部署可选 PostgreSQL（文档）、RustFS 等 S3 兼容对象存储（附件），两者可自由组合
...
- 文件内容存储在后端可选的附件存储：本地文件系统（默认）或 S3 兼容对象存储（RustFS），
  两者共用同一套对象键 `{workspace_id}/{entry_code}/{id}_{文件名}`，因此迁移只需复制目录/对象，无需迁移脚本
...
- Owner/Maintainer 可在 Workspace 设置中查看历史
- 系统管理员可查看全局审计日志
